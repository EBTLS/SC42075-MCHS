function model = MLD_Model_3delta()
    ...
    % basics functions
    f1 = Ts * (2.4733 * u - 0.0108 * v) + v;
    f2 = Ts * (1.6880 * u - 0.0108 * v) + v;
    f3 = Ts * (1.6880 * u - 0.0469 * v + 1.0409) + v;
    f4 = Ts * (1.2812 * u - 0.0469 * v + 1.0409) + v;
...
%     epsilon = 0.0001;
    ...
    % d1 -> v - v1 <= 0
    g = [];
    temp_g = (v - v1) <= (M - v1) * (1 - d1);
    g = [g; temp_g];
    temp_g = (v - v1) >= epsilon + (m - v1 - epsilon) * d1;
    g = [g; temp_g];
    ...
    % d2 -> v - v2 <= 0
    g = [];
    temp_g = (v - v2) <= (M -v2) * (1 - d2);
    g = [g; temp_g];
    temp_g = (v - v2) >= epsilon + (m - v2 - epsilon) * d2;
    g = [g; temp_g];
    constraints = [constraints; g];
    fprintf("d2 -> v - v2 <= 0\n");
    pretty(g);
    ...
    % d3 -> v - v3 <= 0
    g = [];
    temp_g = (v - v3) <= (M - v3) * (1 - d3);
    g = [g; temp_g];
    temp_g = (v - v3) >= epsilon + (m -v3 - epsilon) * d3;
    g = [g; temp_g];
    constraints = [constraints; g];
    fprintf("d3 -> v - v3 <= 0\n");
    pretty(g);
    ...
    MLD.E3(2, 1) = -15;
    % d2
    MLD.E3(3, 2) = 11543/400;
    MLD.E3(4, 2) = -11543/300;
    % d3
    MLD.E3(5, 3) = 5543/200;
    MLD.E3(6, 3) = -30;
    % d1*u -> z1
    ...
    MLD.g5(1) = 15 + 8543/200;
    MLD.g5(2) = - 15;
    % d2
    MLD.g5(3) = 11543/400 + 11543/400;
    MLD.g5(4) = - 11543/400;
    % d3
    MLD.g5(5) = 30 + 5543/200;
    MLD.g5(6) = - 30;
    % d1*u -> z1
    MLD.g5(9) = 13/10;
    MLD.g5(10) = 13/10;
